...
__global__
void fillVertexMapKernel(const Map2DfCuda& depths, const Eigen::Matrix3f& depthIntrinsics, Map2DVector3fCuda& vertexMap) {
...
void fillVertexMap(const Map2DfCuda& depths, const Eigen::Matrix3f& depthIntrinsics, Map2DVector3fCuda& vertexMap)
{
    ...
    fillVertexMapKernel<<<blocks, threads>>>(depths, depthIntrinsics, vertexMap);
...
void applyBiliteralFilter(Map2DfCuda& unfilteredMap, Map2DfCuda& filteredMap) {
    const cv::cuda::GpuMat cvUnfilteredMap(unfilteredMap.getNumberOfColumns(), unfilteredMap.getNumberOfRows(), CV_32F, 
                reinterpret_cast<void*>(unfilteredMap.data()));
    cv::cuda::GpuMat cvFilteredMap(filteredMap.getNumberOfColumns(), filteredMap.getNumberOfRows(), CV_32F, 
